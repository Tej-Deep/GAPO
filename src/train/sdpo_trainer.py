...
@dataclass
class SDPOConfig(DPOConfig):
    loss_type: list[str] = field(
        default_factory=lambda: ["sdpo"],
        metadata={
            "help": "Type of loss to use. Possible values are: `'sdpo'`, `'grpo_sdpo'`.",
            "choices": ["sdpo", "grpo_sdpo"]
        },
    )

def extract_prompt(example: dict[str, Sequence]) -> dict[str, Sequence]:
    r"""
    Extracts the shared prompt from a preference data example, where the prompt is implicit
    within the chosen and all rejected completions.

    The example dictionary is expected to have:
    - "chosen": A Sequence (e.g., list or tuple of tokens/strings) for the preferred completion.
    - "rejected": A List of Sequences (e.g., list of lists/tuples of tokens/strings)
      for the negative samples.
    """
    
    # 1. Collect all completions (chosen + all rejected) into a single list
    all_completions = [example["chosen"]] + example["rejected"]
    
    # 2. Find the length of the shortest completion. The prompt cannot be longer than this.
    min_len = min(len(comp) for comp in all_completions)
    
    # 3. Iterate through indices to find the first token where not all completions match
    common_prefix_length = 0
    for idx in range(min_len):
        # Get the token at the current index from the first completion (chosen)
        current_token = all_completions[0][idx]
        
        # Check if this token matches the token at the same index in ALL other completions
        is_common = all(
            completion[idx] == current_token for completion in all_completions[1:]
        )
        
        if is_common:
            common_prefix_length += 1
        else:
            # First mismatch found, break the loop
            break

    # 4. Apply the optional space removal logic from the original function
    # Check if the token *before* the prompt break is a space.
    if (
        common_prefix_length > 0
        and all_completions[0][common_prefix_length - 1] == " "
    ):
        common_prefix_length -= 1
        
    # 5. Split the completions based on the determined prompt length
    prompt = example["chosen"][:common_prefix_length]
    
    new_chosen = example["chosen"][common_prefix_length:]
    
    new_rejected = [
        rejected_sample[common_prefix_length:]
        for rejected_sample in example["rejected"]
    ]
...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        num_rejected = len(examples[0]["rejected_input_ids"])
        
        batched_rejected_ids = []
        batched_rejected_mask = []
        ...
        for i in range(num_rejected):
            ...
            for example in examples:
                seq = torch.tensor(example["rejected_input_ids"][i])
                mask = torch.ones_like(seq)

                current_batch_ids.append(seq)
                current_batch_masks.append(mask)
            
            # Pad this specific "slice" of rejections independently
            # This is where the efficiency gain happens!
            batched_rejected_ids.append(pad(current_batch_ids, padding_value=self.pad_token_id))
            batched_rejected_mask.append(pad(current_batch_masks, padding_value=0))
        ...
        output["rejected_input_ids"] = batched_rejected_ids
        output["rejected_attention_mask"] = batched_rejected_mask
...
class SDPOTrainer(Trainer):
    ...
    def tokenize_row(
        ...
        rejected_input_ids = []
        for rej_sample in features["rejected"]:
            rejected_input_ids.append(tokenizer(rej_sample, add_special_tokens=False)["input_ids"])
        ...
        for idx in range(len(rejected_input_ids)):
            rejected_input_ids[idx] = rejected_input_ids[idx] + [tokenizer.eos_token_id]
        ...
        if max_completion_length is not None:
            ...
            for idx in range(len(rejected_input_ids)):
                rejected_input_ids[idx] = rejected_input_ids[idx][:max_completion_length] # + [tokenizer.eos_token_id]
    ...
    def concatenated_inputs(
        ...
        """
        output = {}
        num_rejected_batches = len(batch["rejected_input_ids"])
        
        # The total stack multiplier is 1 (for chosen) + N (for rejections)
        total_stack_size = 1 + num_rejected_batches

        # For the prompt, the input_ids are the same for both the chosen and rejected responses
        output["prompt_input_ids"] = torch.cat([batch["prompt_input_ids"]] * total_stack_size, dim=0)
        output["prompt_attention_mask"] = torch.cat(
            [batch["prompt_attention_mask"]]* total_stack_size, dim=0
        )
        ...
        max_chosen_len = batch["chosen_input_ids"].shape[1]
        max_rejected_len = max(t.shape[1] for t in batch["rejected_input_ids"])
        max_completion_length = max(max_chosen_len, max_rejected_len)

        # Prepare the list of tensors to stack: [Chosen, Rej_0, Rej_1...]
        tensors_to_stack = [batch["chosen_input_ids"]] + batch["rejected_input_ids"]
        masks_to_stack = [batch["chosen_attention_mask"]] + batch["rejected_attention_mask"]

        # Pad all tensors to the global max length and concatenate
        padded_inputs = [
            pad_to_length(t, max_completion_length, pad_value=padding_value) 
            for t in tensors_to_stack
        ]
        padded_masks = [
            pad_to_length(t, max_completion_length, pad_value=0) 
            for t in masks_to_stack
        ]

        # Concatenate vertically
        output["completion_input_ids"] = torch.cat(padded_inputs, dim=0)
        output["completion_attention_mask"] = torch.cat(padded_masks, dim=0)
    ...
    def sdpo_loss(
        ...
        rejected_logps: Union[torch.FloatTensor, List[torch.FloatTensor]],
        ref_chosen_logps: torch.FloatTensor,
        ref_rejected_logps: Union[torch.FloatTensor, List[torch.FloatTensor]],
        loss_type: str = "sdpo",
        model_output: dict[str, torch.FloatTensor] = None,
    ) -> tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """
        Compute the DPO loss. 
        Supports standard (1-to-1) DPO and Multi-Negative (1-to-N) DPO.
        """
        device = self.accelerator.device

        # 1. Calculate Chosen Rewards
        # R(e_p) = beta * (pi(p) - ref(p))
        chosen_logratios = chosen_logps.to(device) - (not self.reference_free) * ref_chosen_logps.to(device)
        chosen_rewards = self.beta * chosen_logratios
        
        # 2. Calculate Rejected Rewards for each negative sample
        # R(e_d) = beta * (pi(d) - ref(d))
        rejected_rewards_list = []
        
        for key in range(len(rejected_logps)):
            # Extract current negative batch
            # Depending on your implementation, reference might be a dict or repeated tensor. 
            # Assuming ref_rejected_logps is also a dict based on previous logic.
            current_rej_logps = rejected_logps[key].to(device)
            current_ref_rej_logps = ref_rejected_logps[key].to(device)
            
            rej_logratios = current_rej_logps - (not self.reference_free) * current_ref_rej_logps
            rejected_rewards_list.append(self.beta * rej_logratios)

        # Cross Entropy implementation (equivalent but more stable)
        # 3. Stack all Rewards to create "Logits"
        # We put chosen_rewards first (Index 0)
        # Shape: (Batch_Size, 1 + Num_Negatives)
        # Example: [[Reward_Chosen, Reward_Rej1, Reward_Rej2], ...]
        s_theta = torch.stack([chosen_rewards] + rejected_rewards_list, dim=1)

        if loss_type == "sdpo":
            # Standard SDPO: Logits are just the raw rewards
            logits = s_theta

        elif loss_type == "grpo_sdpo":
            # GRPO-SDPO: Logits are Advantages (s_theta - Baseline)
            
            # A. Compute Total LogSumExp per row (T)
            # T = log( sum( exp(s_i) ) )
            total_logsumexp = torch.logsumexp(s_theta, dim=1, keepdim=True)

            # B. Compute Baseline (B_e) efficiently
            # B_e = T + log(1 - exp(s_e - T))
            # s_theta - total_logsumexp is always <= 0, so exp() is <= 1
            diff = s_theta - total_logsumexp
            
            # Clamp for safety (log(0) protection), though theoretically (1 - exp(diff)) > 0
            # because one element cannot equal the sum of all positive exponentials of the group
            # unless it's the ONLY element (impossible here) or others are -inf.
            safe_term = 1 - torch.exp(diff)
            safe_term = torch.clamp(safe_term, min=1e-6) 
            
            baselines = total_logsumexp + torch.log(safe_term)
            
            # C. Compute Advantage
            logits = s_theta - baselines

        else:
            raise ValueError(f"Unknown loss_type: {loss_type}. Supported: 'sdpo', 'grpo_sdpo'")

        # 4. Compute Cross Entropy Loss
        # The target label is always 0 (the first column is the chosen response)
        labels = torch.zeros(logits.shape[0], dtype=torch.long, device=device)
        
        # CrossEntropyLoss combines LogSoftmax and NLLLoss in one stable step
        # reduction='none' ensures we return the loss per sample, matching TRL API
        loss_fn = torch.nn.CrossEntropyLoss(reduction='none')
        losses = loss_fn(logits, labels)

        # 5. Prepare Return Stats
        # Calculate mean rejected reward for logging
        with torch.no_grad():
            stacked_rejected = torch.stack(rejected_rewards_list, dim=1)
            mean_rejected_rewards = stacked_rejected.mean(dim=1)

        return losses, chosen_rewards.detach(), mean_rejected_rewards.detach()

        # Formula Wise Implementation
        # # 3. Stack Rejected Rewards -> Shape: (Batch, Num_Negatives)
        # stacked_rejected_rewards = torch.stack(rejected_rewards_list, dim=1)
        
        # # 4. Compute the Difference Terms (Margins)
        # # diff = R(e_d) - R(e_p)
        # # chosen_rewards is (Batch,), we unsqueeze to broadcast against (Batch, Num_Neg)
        # diffs = stacked_rejected_rewards - chosen_rewards.unsqueeze(1)
        
        # # 5. Compute Loss using the derived LogSumExp trick
        # # Loss = log(1 + sum(exp(diffs)))
        # # We concatenate a column of zeros to the diffs matrix. 
        # # Because logsumexp([0, x, y]) = log(e^0 + e^x + e^y) = log(1 + e^x + e^y)
        # batch_size = chosen_logps.shape[0]
        # zeros = torch.zeros(batch_size, 1, device=device)
        
        # # Shape: (Batch, 1 + Num_Negatives)
        # concat_diffs = torch.cat([zeros, diffs], dim=1)
        
        # # Apply LogSumExp
        # losses = torch.logsumexp(concat_diffs, dim=1)

        # # For logging purposes, we can return the mean rejected reward
        # mean_rejected_rewards = stacked_rejected_rewards.mean(dim=1)
        
        # return losses, chosen_rewards, mean_rejected_rewards
    ...
    def concatenated_forward(
        ...
        rejected_logps_ls = []
        num_rejected_lists = len(batch["rejected_input_ids"])
        
        for i in range(num_rejected_lists):
            start = num_examples * (i + 1)
            end = num_examples * (i + 2)
            rejected_logps_ls.append(all_logps[start:end])
            
        output["rejected_logps"] = rejected_logps_ls
        ...
        with torch.no_grad():
            ...
            output["mean_rejected_logits"] = logits[num_examples : 2*num_examples][loss_mask[num_examples : 2*num_examples]].mean()
    ...
    def get_batch_loss_metrics(
        ...
        if self.args.use_liger_loss:
            ...
            for idx, loss_type in enumerate(self.loss_type):
                ...
                _losses, _chosen_rewards, _rejected_rewards = self.sdpo_loss(
        ...
        # Changed to just log 1st rejection logps
        metrics[f"{prefix}logps/rejected"] = (
            self.accelerator.gather_for_metrics(model_output["rejected_logps"][0]).detach().mean().item()
        )
